...
export async function getNewsArticles(limit: number = 4, locale?: string): Promise<NewsArticle[]> {
  const params = new URLSearchParams({ limit: limit.toString(), sort: '-publishedDate', depth: '1' });
  if (locale) params.append('locale', locale);
...
export async function getInstagramPostsFromPayload(limit: number = 5): Promise<PopulatedInstagramPost[]> {
  const queryParams = new URLSearchParams({ sort: '-postDate', limit: limit.toString(), depth: '1' });
  try {
    const data = await fetchAPI<CollectionResponse<PopulatedInstagramPost>>(`/instagram-posts?${queryParams.toString()}`, {}, ['instagram-posts']);
    return data.docs || [];
  } catch (error) {
